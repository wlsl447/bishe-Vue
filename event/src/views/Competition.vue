...
<template>
  <div>
    <el-table
      :data="tableData"
      border
      :default-sort="{ prop: 'cpstime', order: 'descending' }"
    >
      <!-- style="width: 100%" -->
      <!-- <el-table-column
        fixed
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="addusercp(tableDatafrom.id)"
          >报名</el-button
        >
        <!-- <el-button type="primary" @click="dialogVisible = false"
          >确 定</el-button
        ...
        .then((res) => {
          ...
          if (res.data.code === 200) {
            if (res.data.data === "该赛事是团队赛需要队伍报名") {
              return this.$confirm("该赛事是团队赛需要队伍报名", "提示", {
                confirmButtonText: "去创建队伍",
                cancelButtonText: "去招募大厅加队伍",
                type: "warning",
              })
                .then(() => {
                ...
                .catch(() => {
                  this.$router.push("/recruitteam");
                });
              // return alert("该赛事是团队赛需要队伍报名");
            }
            if(res.data.data === "该赛事已经结束！"){
              return alert("该赛事已经结束！");
            }
            alert("报名成功");
