        ...
        <el-form ref="tableDatafrom" :model="dialogData" label-width="120px">
          ...
          <el-form-item label="获奖文件">
            <!-- <el-upload
              ...
              :auto-upload="false"
            >
              <el-button size="small" type="primary" v-show="isAdd">点击上传</el-button>
              <div slot="tip" class="el-upload__tip" v-show="isAdd">
                只能上传jpg/docx/doc/excel文件,仅限一份
              </div>
            </el-upload> -->
            <!-- <el-link type="primary"  @click="downloadFile(dialogData.url)">{{ dialogData.name }}</el-link> -->
            <el-link type="primary" :href="dialogData.url" :download="'测试'">{{ dialogData.name }}</el-link>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    uploadRequest(data) {
      this.dialogData.file = data.file;
    },
    ...
    addAward(res) {
      ...
      // console.log(this.tableData.awardName)
      Object.keys(this.dialogData).forEach((key) => {
        formData.append(key, this.dialogData[key]);
      });
      // formData.append("awardName",this.dialogData.awardName)
      // formData.append("file",this.dialogData.file)
      console.log("formdata", formData);
      this.$axios.post("/addAward", formData).then(({ data }) => {
        if (data.code === 200) {
          alert("颁奖成功");
          this.dialogVisible = false;
          this.getMyAward();
    ...
    handleClick(row) {
      ...
      this.dialogData.url = row.url;
      // if (this.form.name != "" && this.form.url != "") {
      //   // console.log(this.form.name);
        this.fileList = [];
        this.fileList.push(this.dialogData);
      // }
    },
    handleUpdate(res) {
      // this.dialogVisible = true;
