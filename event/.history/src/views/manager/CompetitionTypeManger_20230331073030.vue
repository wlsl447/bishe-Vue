...
<template>
  <div>
    <el-table :data="placeData" border>
      ...
      <el-table-column prop="address" label="赛事类型"> </el-table-column>
      ...
      <el-table-column label="操作">
        ...
        <template slot-scope="scope">
          ...
          <el-button
            type="danger"
            @click="deleteAddressRoom(scope.row)"
            size="small"
            >删除</el-button
          >
        </template>
    ...
    >
      ...
      <el-table :data="roomData" border>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="danger" @click="deleteRoom(scope.row)" size="small"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    //添加场地具体位置
    addRoom(placeForm) {
      console.log(JSON.parse(placeForm.id));
      this.$axios
        .post("/addRoom", {
          id: placeForm.id,
          room: placeForm.room,
    ...
    getRoomId(ids) {
      ...
      this.$axios
        .get("/getAddressRoom", {
          params: {
            placeId: JSON.parse(ids.placeId),
          },
        })
        .then(({ data }) => {
          console.log(data);
