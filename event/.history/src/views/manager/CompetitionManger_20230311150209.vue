...
<template>
  <div>
    ...
    >
      <div>
        <el-form ref="tableDatafrom" :model="tableDatafrom" label-width="120px">
          ...
          <el-form-item label="个人赛·团体赛">
            ...
            ></el-input > -->
            <template >
              <el-radio v-model="tableDatafrom.teamType" label="团队赛" :disabled="isInput"
                >团队赛</el-radio
              >
              <el-radio v-model="tableDatafrom.teamType" label="个人赛" :disabled="isInput"
                >个人赛</el-radio
              >
            </template>
          </el-form-item>
          ...
          <el-form-item label="状态">
            ...
            ></el-input> -->
            <template >
              <el-radio v-model="tableDatafrom.cpstate" label="未开始" :disabled="isInput"
                >未开始</el-radio
              >
              <el-radio v-model="tableDatafrom.cpstate" label="进行中" :disabled="isInput"
                >进行中</el-radio
              >
              <el-radio v-model="tableDatafrom.cpstate" label="已结束" :disabled="isInput"
                >已结束</el-radio
              >
            </template>
          </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="dialogVisible = false" v-show="isConfirm"
          >确 定</el-button
        >
        <el-button type="primary" @click="competitionCreate(tableDatafrom)" v-show="isCreate"
          >确认创建</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    competitionCreate(data){
      // console.log(data);
      this.$axios.post("/competitionCreate",{
        data:data
      }).then(({data})=>{
        console.log(data);
      }) .catch((resp) => {
    ...
    createCompetition() {
      ...
      console.log(this.$refs.dialogCp.title);
    },
