    ...
    <el-aside width="200px">
      <div v-for="(item, index) in mydata" :key="item.noticeId">
        <spen
          ...
          :class="carouseId == item.id ? 'isActive1' : 'isActive1'"
          >{{ item.title }}</spen
        >
        <el-tabs v-model="activeName" @tab-click="handleClick" :tab-position="tabPosition" v-for="(item, index) in mydata" :key="item.noticeId">
          <el-tab-pane label="item" name="first">{{item.title}}</el-tab-pane>
        </el-tabs>
      </div>
    </el-aside>
    <el-main>
      <div>
        ...
        >
          <el-carousel-item v-for="item in mydata" :key="item.noticeId">
            <el-descriptions title="系统公告">
              <el-descriptions-item label="用户名" :formatter="dateFormat">{{
                item.time
